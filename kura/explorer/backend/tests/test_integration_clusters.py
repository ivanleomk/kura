...
class TestClusterConversationMapping:
    ...
    def test_database_has_conversations(self, real_explorer):
        ...
        with Session(real_explorer.engine) as session:
            ...
            assert total_conversations > 0, f"Expected conversations, found {total_conversations}"
    ...
    def test_database_has_cluster_conversation_links(self, real_explorer):
        ...
        with Session(real_explorer.engine) as session:
            ...
            assert link_count > 0, f"Expected cluster-conversation links, found {link_count}"
    ...
    def test_conversation_ids_match_summaries(self, real_explorer):
        ...
        with Session(real_explorer.engine) as session:
            ...
            assert summary is not None, f"No summary found for conversation {conversation.chat_id}"
    ...
    def test_cluster_has_valid_conversation_data(self, real_explorer):
        ...
        assert cluster_detail is not None, f"Could not retrieve cluster {cluster_with_convs.id}"
        ...
        if hasattr(cluster_detail, 'conversations') and cluster_detail.conversations:
            ...
            print("   - No conversations loaded (this indicates the relationship issue)")
...
class TestClusterAPIEndpoints:
    ...
    def test_api_clusters_list(self, real_client):
        ...
        print(f"   - First cluster: '{cluster['name']}' with {cluster['conversation_count']} conversations")
    ...
    def test_api_cluster_tree_structure(self, real_client):
        ...
        for root_cluster in data:
            ...
            print(f"✅ Root cluster: '{root_cluster['name']}' with {root_cluster['conversation_count']} conversations")
            
            # Check children
            for child in root_cluster['children'][:2]:  # Check first 2 children
                assert "id" in child
                assert "name" in child
                print(f"   - Child: '{child['name']}' with {child['conversation_count']} conversations")
    ...
    def test_orphaned_cluster_links(self, real_explorer):
        ...
        with Session(real_explorer.engine) as session:
            ...
            orphaned_links = session.exec(
                select(ClusterConversationLink)
                .outerjoin(ConversationDB, ClusterConversationLink.conversation_id == ConversationDB.chat_id)
                .where(ConversationDB.chat_id.is_(None))
            ).all()
            
            if len(orphaned_links) > 0:
                print(f"⚠️  Found {len(orphaned_links)} orphaned cluster-conversation links")
    ...
    def test_conversations_without_summaries(self, real_explorer):
        ...
        with Session(real_explorer.engine) as session:
            ...
            if len(conversations_without_summaries) > 0:
                print(f"⚠️  Found {len(conversations_without_summaries)} conversations without summaries")
            else:
        ...
        with Session(real_explorer.engine) as session:
            ...
            for cluster in clusters:
                ...
                if cluster_detail:
                    ...
                    print(f"Cluster '{cluster.name}': reported={reported_count}, actual={actual_count}")
