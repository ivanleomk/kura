...
class TestConversationFiltering:
    ...
    def test_conversation_filtering_with_summary(self, client, mock_explorer):
        ...
        mock_explorer.get_conversations.return_value = [
            conv_with_python,
            conv_with_javascript,
        ]

        # Mock summaries with different languages
        def mock_get_summary(chat_id):
            if chat_id == "conv_python":
                return Mock(languages=["python"], user_frustration=3, task="coding")
            elif chat_id == "conv_js":
                return Mock(
                    languages=["javascript"], user_frustration=2, task="debugging"
                )
            return None
