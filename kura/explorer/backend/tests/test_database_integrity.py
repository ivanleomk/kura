    ...
    def test_required_tables_exist(self, checkpoint_explorer):
        ...
        for table in required_tables:
            assert table in tables, (
                f"Required table '{table}' not found. Found: {tables}"
            )
    ...
    def test_table_schemas(self, checkpoint_explorer):
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            for col in expected_conv_columns:
                assert col in conv_columns, (
                    f"Column '{col}' missing from conversations table"
                )
            ...
            for col in expected_link_columns:
                assert col in link_columns, (
                    f"Column '{col}' missing from cluster_conversations table"
                )
...
class TestDataLoading:
    ...
    def test_conversations_loaded_from_summaries(self, checkpoint_explorer):
        ...
        with Session(checkpoint_explorer.engine) as session:
            ...
            assert conv_count >= summary_count, (
                f"Fewer conversations ({conv_count}) than summaries ({summary_count})"
            )
    ...
    def test_conversation_ids_match_summary_ids(self, checkpoint_explorer):
        ...
        with Session(checkpoint_explorer.engine) as session:
            ...
            if missing_conversations:
                print(
                    f"⚠️  Summaries without conversations: {len(missing_conversations)}"
                )
            ...
            assert len(missing_conversations) == 0, (
                f"Found {len(missing_conversations)} summaries without conversations"
            )

            print(
                f"✅ All {len(summary_ids)} summaries have corresponding conversations"
            )
            ...
            for link in links[:100]:  # Check first 100 links
                ...
                conversation = session.exec(
                    select(ConversationDB).where(
                        ConversationDB.chat_id == link.conversation_id
                    )
                ).first()
    ...
    def test_cluster_conversations_relationship(self, checkpoint_explorer):
        ...
        with Session(checkpoint_explorer.engine) as session:
            ...
            try:
                ...
                print(
                    f"✅ Cluster '{cluster.name}' relationship loaded {len(conversations)} conversations"
                )

                # Verify conversations are actual ConversationDB objects
                for conv in conversations[:3]:
                    assert hasattr(conv, "chat_id"), (
                        "Conversation object missing chat_id"
                    )
                    assert hasattr(conv, "created_at"), (
                        "Conversation object missing created_at"
                    )
                ...
                print(
                    f"⚠️  Manual join found {len(conversations)} conversations for cluster"
                )
    ...
    def test_conversation_clusters_relationship(self, checkpoint_explorer):
        ...
        with Session(checkpoint_explorer.engine) as session:
            ...
            try:
                ...
                print(
                    f"✅ Conversation {conversation.chat_id[:8]}... belongs to {len(clusters)} clusters"
                )
                ...
                clusters = session.exec(
                    ...
                    .join(ClusterConversationLink)
                    .where(
                        ClusterConversationLink.conversation_id == conversation.chat_id
                    )
                ).all()
...
class TestAPIDataConsistency:
    ...
    def test_get_cluster_conversation_consistency(self, checkpoint_explorer):
        ...
        assert cluster_detail is not None, (
            f"Could not get cluster detail for {cluster_id}"
        )
        ...
        with Session(checkpoint_explorer.engine) as session:
            ...
            loaded_convs = (
                len(cluster_detail.conversations)
                if hasattr(cluster_detail, "conversations")
                and cluster_detail.conversations
                else 0
            )
            ...
            with Session(checkpoint_explorer.engine) as session:
                cluster_links = session.exec(
                    select(ClusterConversationLink).where(
                        ClusterConversationLink.conversation_id == conv.chat_id
                    )
                ).all()
