...
from sqlmodel import Field, SQLModel, Session, Relationship, create_engine, Column, DateTime, JSON


# Link table for many-to-many relationship
class ClusterConversationLink(SQLModel, table=True):
    __tablename__ = "cluster_conversations"  # type: ignore[assignment]
...
class ConversationDB(SQLModel, table=True):
    ...
    clusters: List["ClusterDB"] = Relationship(back_populates="conversations", link_model=ClusterConversationLink)
...
class ClusterDB(SQLModel, table=True):
    __tablename__ = "clusters"  # type: ignore[assignment]
    ...
    conversations: List[ConversationDB] = Relationship(back_populates="clusters", link_model=ClusterConversationLink)
